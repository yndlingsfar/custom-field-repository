...
class Field_Reader
{
	 ...
	 * @return bool
	 */
    public function is_annotated_field($property)
    {
	 ...
	 * @return string
	 */
    public function get_field_name($property)
    {
	    $annotations = $this->annotations->getPropertyAnnotations($this->field_group, $property);
	    if ( ! is_array( $annotations ) || ! array_key_exists( self::KEY_NAME, $annotations['Field'][0] ) ) {
		    throw new Proxy_Exception( sprintf( 'Field %s not configured correctly', $property ) );
	    }
    ...
    public function get_field_group_name()
    {
	    ...
	    if ( ! is_array( $annotations ) || ! array_key_exists( 'name', $annotations['Field_Group'][0] ) ) {
		    throw new Proxy_Exception( sprintf( 'Field_Group %s not configured correctly', $this->field_group ) );
	    }
